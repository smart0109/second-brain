...
async function getOnlineMeetingByJoinUrl(accessToken, joinUrl) {
  ...
  // $filter on JoinWebUrl. Escape single quotes per OData (double them) to prevent
  // filter injection, and only accept Microsoft Teams join URLs.
  if (!/^https:\/\/teams\.microsoft\.com\//i.test(joinUrl)) {
    throw new Error('getOnlineMeetingByJoinUrl: joinUrl must be an https://teams.microsoft.com/ URL');
  }
  const safe = String(joinUrl).replace(/'/g, "''");
  const filter = `JoinWebUrl eq '${safe}'`;
