...
// ---------------------------------------------------------------------------
// Comma-separated domains (e.g. "cadienttalent.com,vorro.net,basisvps.com") whose
// users may self-sign-up. A leading @ is tolerated.
...
async function isAllowed(email) {
  ...
  if (domain && allowedDomains().includes(domain)) return true; // domain allow = sign-up
...
async function purgeUserSessions(email) {
  // Best-effort hard logout: delete this user's stored sessions so removal takes
  // effect immediately. connect-pg-simple stores session data as JSON in "session".
  const e = (email || '').toLowerCase();
  await db.query(`DELETE FROM "session" WHERE (sess->>'email') = $1`, [e]);
}
